...
const InvoiceToClaim = () => {
    ...
    const handleCreateNew = () => {
        navigation.navigate(''); // Assuming a route for creating a new claim
    };

    const handleView = (item) => {
        navigation.navigate('InvoicetoClaimOverview');
    };

    // The handleDelete function is no longer needed but kept for reference
            ...
            renderCell: (item) => (
                <View style={styles.actionCell}>
                    <TouchableOpacity onPress={() => handleView(item)} style={{ marginRight: 10 }}>
                      ...
                      <TouchableOpacity onPress={() => handleDelete(item)} style={{ marginRight: 10 }}>
                        <Ionicons name="eye-outline" size={20} color="#007bff" />
                    </TouchableOpacity>
        ...
        <SafeAreaView style={styles.safeArea}>
            <ScrollView style={styles.container}>
                <View style={styles.headerRow}>
                    <Text style={styles.title}>Invoice To Claim</Text>
                    <AppButton
                        label="+ Create new"
